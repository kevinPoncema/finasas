<script lang="ts">
    ...
    let addedFilters: { name: string; type: string; field: string; options?: string[] }[] = [];
    ...
    const handleFieldChange = (index: number, value: string) => {
      addedFilters[index].field = value;
    };
  ...
  <div class="p-2 bg-white shadow-md rounded-lg">
    ...
    <div class="flex justify-center items-center space-x-2">
      <button class="bg-blue-500 text-white px-3 py-1 rounded-md text-sm" on:click={crarITem}>
        + Agregar {itemName}
      </button>
